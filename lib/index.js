...
function authorize(options) {
  ...
  if (!auth.cookieParser && !auth.cookieHelper) {
    try {
  ...
  return async function(data, accept) {
    ...
    if(auth.cookieHelper) {
      data.sessionID = auth.cookieHelper.get(socket, auth.key, [auth.secret]);
    } else {
    ...
    let session;
    try {
      session = await auth.store.get(data.sessionID);
    } catch(err) {
